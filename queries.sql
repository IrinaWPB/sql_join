...
SELECT first_name, last_name, ROUND(AVG(price)) AS average_price, COUNT(*) FROM owners o JOIN vehicles v ON o.id = v.owner_id GROUP BY o.id HAVING COUNT(*) > 1 and ROUND(AVG(price)) > 10000 ORDER BY first_name DESC;
